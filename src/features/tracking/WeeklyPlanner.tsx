...
import { CalendarDays, ChevronDown, ChevronUp, ClipboardList, Dumbbell, Pencil, Plus, Trash2 } from "lucide-react";
...
function itemTitle(item: PlannedItem) { return typeof item === "string" ? item : item.title; }
...
export function WeeklyPlanner() {
  ...
  const editTask = (day: Day, index: number) => { const replacement = window.prompt("Modifier la session", itemTitle(schedule[day][index])); if (!replacement?.trim()) return; const tasks = [...schedule[day]]; const current = tasks[index]; tasks[index] = typeof current === "string" ? replacement.trim() : { ...current, title: replacement.trim() }; updateDay(day, tasks); };
