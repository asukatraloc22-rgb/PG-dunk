...
type CustomWorkoutBuilderProps = { onSave: (workout: Workout) => Promise<void> };

const emptyExercise = (): Exercise => ({ name: "", sets: 3, reps: "8", restSeconds: 60, notes: "" });

export function CustomWorkoutBuilder({ onSave }: CustomWorkoutBuilderProps) {
  const [title, setTitle] = useState("Ma routine RIZE");
  const [description, setDescription] = useState("");
  const [focusArea, setFocusArea] = useState("Ball handling");
  ...
  return <section className="space-y-4 rounded-[2rem] border border-orange-300/15 bg-slate-900/70 p-4 shadow-xl shadow-black/10 backdrop-blur-xl sm:p-6">
    <div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-300/70">Builder</p><h3 className="mt-1 text-xl font-black text-white">Créer ma séance</h3><p className="mt-1 text-sm leading-relaxed text-slate-500">Construis une routine adaptée à ton objectif, puis retrouve-la dans ta bibliothèque.</p></div>
