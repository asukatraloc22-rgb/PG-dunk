            ...
            <div className="space-y-6">
              <div className="rounded-[1.75rem] border border-white/10 bg-slate-900/65 p-4 shadow-xl shadow-black/10 backdrop-blur-xl sm:p-6">
                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {showIqFlashcard && selectedIqLesson && <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-label={`Fiche ${selectedIqLesson.title}`}><div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-[2rem] border border-orange-300/20 bg-[#0b1b32] p-5 shadow-2xl shadow-black/50 sm:rounded-[2rem] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-orange-300/70">Flashcard IQ · {selectedIqLesson.category}</p><h3 className="mt-2 text-2xl font-black tracking-[-0.04em] text-white">{selectedIqLesson.title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-300">{selectedIqLesson.summary}</p></div><button type="button" onClick={() => setShowIqFlashcard(false)} className="rounded-xl bg-white/5 p-2 text-slate-400 hover:bg-white/10 hover:text-white" aria-label="Fermer la flashcard"><X size={18} /></button></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-orange-300/15 bg-orange-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-orange-300/70">Principes de jeu</p><div className="mt-3 flex flex-wrap gap-2">{selectedIqLesson.principles.map((principle) => <span key={principle} className="rounded-lg bg-white/5 px-2.5 py-1.5 text-xs font-semibold text-slate-200">{principle}</span>)}</div></div><div className="rounded-2xl border border-sky-300/15 bg-sky-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-sky-300/70">American vocabulary</p><p className="mt-3 text-sm leading-relaxed text-slate-300">{selectedIqLesson.vocabulary?.join(" · ") || "Terminologie à compléter dans le glossaire."}</p></div></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-300/70">Coach cues</p><ul className="mt-2 space-y-2 text-sm leading-relaxed text-slate-300">{(selectedIqLesson.coachCues ?? selectedIqLesson.principles).map((cue) => <li key={cue}>↳ {cue}</li>)}</ul></div><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-rose-300/70">Common mistakes</p><ul className="mt-2 space-y-2 text-sm leading-relaxed text-slate-300">{(selectedIqLesson.commonErrors ?? []).map((error) => <li key={error}>× {error}</li>)}</ul></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-emerald-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-300/70">Solo transfer</p><p className="mt-2 text-sm leading-relaxed text-slate-300">{selectedIqLesson.soloPractice || "Visualise la situation puis répète les deux côtés du terrain."}</p></div><div className="rounded-2xl bg-violet-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-300/70">Team transfer</p><p className="mt-2 text-sm leading-relaxed text-slate-300">{selectedIqLesson.teamPractice || "Répéter sans défense, puis ajouter la couverture et la lecture."}</p></div></div>{selectedIqLesson.relatedPlayIds && selectedIqLesson.relatedPlayIds.length > 0 && <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/35 p-4"><div className="flex items-center justify-between gap-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-orange-300/70">Playbook associé</p><button type="button" onClick={() => { const relatedPlay = PLAYBOOK_PLAYS.find((play) => selectedIqLesson.relatedPlayIds?.includes(play.id)); if (relatedPlay) { setSelectedPlay(relatedPlay); setActiveTab("playbook"); setPlayView("choose"); setShowIqFlashcard(false); } }} className="text-xs font-black text-orange-200 hover:text-white">Ouvrir le playbook →</button></div><div className="mt-3 flex flex-wrap gap-2">{selectedIqLesson.relatedPlayIds.map((playId) => <span key={playId} className={`rounded-lg px-2.5 py-1.5 text-xs font-bold ${COACHBOARD_PLAY_IDS.has(playId) ? "bg-sky-500/15 text-sky-200" : "bg-white/5 text-slate-300"}`}>{playId}{COACHBOARD_PLAY_IDS.has(playId) ? " · Coachboard" : ""}</span>)}</div></div>}</div></div>}
