...
function App() {
  ...
  const coachboardAction = activeCoachFrame.title.toLowerCase().includes("screen") || activeCoachFrame.title.toLowerCase().includes("écran") ? "Écran" : activeCoachFrame.title.toLowerCase().includes("roll") ? "Roll" : activeCoachFrame.title.toLowerCase().includes("pop") ? "Pop" : activeCoachFrame.title.toLowerCase().includes("tir") ? "Shot" : "Cut / move";
            ...
            <div className="space-y-6">
              <div className="rounded-[1.75rem] border border-white/10 bg-slate-900/65 p-4 shadow-xl shadow-black/10 backdrop-blur-xl sm:p-6">
                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="mt-4 divide-y divide-white/5 rounded-2xl border border-white/10 bg-slate-950/25">{filteredIQLessons.map((lesson) => <button type="button" key={lesson.id} onClick={() => setSelectedIqLessonId(lesson.id)} className={`flex w-full items-center justify-between gap-3 px-3.5 py-3 text-left transition-colors first:rounded-t-2xl last:rounded-b-2xl ${selectedIqLesson?.id === lesson.id ? "bg-orange-500/10" : "hover:bg-white/[0.03]"}`}><div className="min-w-0"><p className="truncate text-sm font-black text-white">{lesson.title}</p><p className="mt-0.5 truncate text-[11px] text-slate-500">{lesson.category} · {lesson.format ?? "Leçon pratique"}</p></div><div className="flex shrink-0 items-center gap-2"><span className="text-[10px] font-bold text-slate-600">{lesson.level}</span><span className="text-slate-500">›</span></div></button>)}</div>
          ...
          {activeTab === 'playbook' && (
            <div className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
              ...
              <div className="rounded-[1.75rem] border border-white/10 bg-slate-900/65 p-4 shadow-xl shadow-black/10 backdrop-blur-xl sm:p-6">
                ...
                <AnimatedCoachboard frame={activeCoachFrame} previousFrame={previousCoachFrame} step={playStep} totalSteps={coachboardFrames.length} actionLabel={coachboardAction} />
