            ...
            <div className="space-y-6">
              <div className="rounded-[1.75rem] border border-white/10 bg-slate-900/65 p-4 shadow-xl shadow-black/10 backdrop-blur-xl sm:p-6">
                ...
                <div className="mb-6 rounded-2xl border border-orange-300/15 bg-orange-500/5 p-4">
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">{filteredIQLessons.map((lesson) => <button type="button" key={lesson.id} onClick={() => setSelectedIqLessonId(lesson.id)} className={`rounded-xl border p-3 text-left transition-all ${selectedIqLesson?.id === lesson.id ? "border-orange-300/40 bg-orange-500/10" : "border-white/5 bg-slate-950/35 hover:border-white/15"}`}><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-black uppercase tracking-wider text-orange-300/70">{lesson.category}</span><span className="text-[10px] font-bold text-slate-600">{lesson.level}</span></div><h4 className="mt-1 text-sm font-black text-white">{lesson.title}</h4><p className="mt-1 text-xs leading-relaxed text-slate-500">{lesson.summary}</p><div className="mt-2 flex flex-wrap gap-1">{lesson.principles.slice(0, 3).map((principle) => <span key={principle} className="rounded-md bg-white/5 px-1.5 py-1 text-[10px] text-slate-400">{principle}</span>)}</div></button>)}</div>
                  {selectedIqLesson && <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/45 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-orange-300/70">Fiche pratique · {selectedIqLesson.format ?? "Leçon"}</p><h4 className="mt-1 text-base font-black text-white">{selectedIqLesson.title}</h4></div><span className="rounded-full bg-white/5 px-2 py-1 text-[10px] font-bold text-slate-400">{selectedIqLesson.level}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><p className="text-[10px] font-black uppercase tracking-wider text-slate-600">Consignes coach</p><ul className="mt-2 space-y-1 text-xs text-slate-400">{(selectedIqLesson.coachCues ?? selectedIqLesson.principles).map((cue) => <li key={cue}>↳ {cue}</li>)}</ul></div><div><p className="text-[10px] font-black uppercase tracking-wider text-slate-600">Erreurs à corriger</p><ul className="mt-2 space-y-1 text-xs text-slate-400">{(selectedIqLesson.commonErrors ?? ["Rester passif", "Lire trop tard"]).map((error) => <li key={error}>× {error}</li>)}</ul></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2"><div className="rounded-xl bg-emerald-500/5 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-emerald-300/70">Version solo</p><p className="mt-1 text-xs leading-relaxed text-slate-400">{selectedIqLesson.soloPractice ?? "Visualiser la situation puis répéter le geste des deux côtés."}</p></div><div className="rounded-xl bg-sky-500/5 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-sky-300/70">Version équipe</p><p className="mt-1 text-xs leading-relaxed text-slate-400">{selectedIqLesson.teamPractice ?? "Faire 5 répétitions sans défense puis passer en opposition réelle."}</p></div></div>{selectedIqLesson.pros && selectedIqLesson.cons && <div className="mt-4 grid gap-2 sm:grid-cols-2"><div><p className="text-[10px] font-black uppercase tracking-wider text-emerald-300/70">Forces</p><p className="mt-1 text-xs text-slate-400">{selectedIqLesson.pros.join(" · ")}</p></div><div><p className="text-[10px] font-black uppercase tracking-wider text-rose-300/70">Limites</p><p className="mt-1 text-xs text-slate-400">{selectedIqLesson.cons.join(" · ")}</p></div></div>}</div>}
                </div>
