...
function App() {
  ...
  const dashboardFocus = activeWorkout
    ? { eyebrow: "Session en cours", title: activeWorkout.title, detail: "Reprends exactement là où tu t’es arrêté.", action: "Revenir à la séance", tab: "training" as const }
    : savedWorkouts.length > 0
      ? { eyebrow: "Prochaine action", title: "Relancer une séance utile", detail: "Choisis un workout sauvegardé ou ouvre ton planning.", action: "Ouvrir Entraîner", tab: "training" as const }
      : { eyebrow: "Premier mouvement", title: "Construire ta première séance", detail: "Commence par une routine claire, puis mesure ce que tu fais.", action: "Créer ma séance", tab: "training" as const };
        ...
        <main>
          ...
          {activeTab === 'workouts' && (
            <div className="space-y-6">
              ...
              <section className="rize-dashboard-focus rize-rise-in grid gap-4 rounded-[1.75rem] border border-orange-300/20 bg-gradient-to-br from-slate-900/90 via-slate-900/70 to-orange-950/30 p-4 shadow-2xl shadow-black/15 sm:grid-cols-[1fr_auto] sm:items-end sm:p-5" aria-label="Prochaine action">
                <div>
                  <div className="mb-3 flex items-center gap-2"><span className="rize-status-dot" aria-hidden="true" /><p className="text-[10px] font-black uppercase tracking-[0.18em] text-orange-300">{dashboardFocus.eyebrow}</p></div>
                  <h2 className="max-w-xl text-2xl font-black tracking-[-0.04em] text-white sm:text-3xl">{dashboardFocus.title}</h2>
                  <p className="mt-2 max-w-lg text-sm leading-relaxed text-slate-400">{dashboardFocus.detail}</p>
                </div>
                <button onClick={() => setActiveTab(dashboardFocus.tab)} className="rize-pressable inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-400 px-5 py-3 text-sm font-black text-slate-950 shadow-xl shadow-orange-950/25 transition hover:brightness-110">{dashboardFocus.action}<ChevronUp className="rotate-90" size={16} /></button>
              </section>
              ...
              <section className="rize-rise-in rounded-[1.5rem] border border-white/10 bg-slate-900/65 p-4 shadow-xl shadow-black/10 backdrop-blur-xl sm:p-5" aria-label="Actions rapides">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {[
                    ...
                    <button key={action.id} onClick={() => setActiveTab(action.id as typeof activeTab)} className="rize-pressable group flex min-h-24 items-start gap-3 rounded-2xl border border-white/10 bg-slate-950/35 p-4 text-left transition hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.06] active:scale-[0.99]">
              ...
              <nav className="rize-rise-in flex justify-center gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`rize-pressable px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-medium transition-all ${
                      workoutTab === tab.id
          ...
          {activeTab === 'iq' && (
            <div className="space-y-6">
              ...
              {/* IQ Stats */}
              <div className={`grid min-w-0 gap-4 md:grid-cols-3 ${iqMode === "quiz" ? "" : "hidden"}`}>
