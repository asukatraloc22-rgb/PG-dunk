...
export const COACHBOARD_PLAY_IDS = new Set([
  ...
  "defense-shell", "defense-23-zone", "defense-vs-pnr", "defense-vs-5out", "defense-vs-post", "slob-floppy", "slob-elevator", "slob-safety", "blob-box-screen", "blob-box-lob", "blob-stack-cross", "blob-zipper", "zoom-action", "double-drag", "ghost-pnr", "chicago-action", "horns-elbow-split",
]);
...
export const getCoachboardFrames = (playId: string): CoachFrame[] => {
  switch (playId) {
    ...
    case "zoom-action":
      return [
        makeFrame("Zoom · stagger setup", "Le shooteur démarre dans le corner faible. Les deux screeneurs s’alignent en stagger sans fermer la raquette.", [position("o1", "top"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "block-left"), position("o5", "block-right")], { ...point("top"), owner: "o1" }),
        makeFrame("Curl, fade or backdoor", "Le shooteur vend d’abord la coupe vers le cercle puis choisit curl, fade ou backdoor selon la position du défenseur.", [position("o1", "top"), position("o2", "wing-left"), position("o3", "corner-right"), position("o4", "elbow-left"), position("o5", "block-left")], { ...point("top"), owner: "o1" }, [action("cut", "o2", "wing-left", "Stagger exit"), action("screen", "o4", "o2", "First screen"), action("screen", "o5", "o2", "Second screen")], point("wing-left")),
        makeFrame("Hit the shooter", "Le passeur sert le shooteur dans son rythme. Si la défense top-lock, la coupe backdoor devient la première lecture.", [position("o1", "slot-left"), position("o2", "wing-left"), position("o3", "corner-right"), position("o4", "block-left"), position("o5", "slot-left")], { ...point("wing-left"), owner: "o1" }, [action("pass", "o1", "o2", "Hit shooter"), action("shot", "o2", "rim", "Wing three"), action("cut", "o2", "rim", "Backdoor counter")], point("wing-left")),
      ];
    case "double-drag":
      return [
        makeFrame("Double Drag · early offense", "Le 1 remonte au top avec trois couloirs ouverts. Les deux screeneurs arrivent en succession, pas côte à côte.", [position("o1", "top"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "wing-left"), position("o5", "wing-right")], { ...point("top"), owner: "o1" }),
        makeFrame("Two screens in a row", "Le premier écran ralentit le défenseur du porteur ; le second arrive avec un angle différent pour prolonger l’avantage.", [position("o1", "slot-right"), position("o4", "top"), position("o5", "elbow-right"), position("o2", "corner-left"), position("o3", "corner-right")], { ...point("top"), owner: "o1" }, [action("screen", "o4", "o1", "First drag"), action("screen", "o5", "o1", "Second drag"), action("dribble", "o1", "slot-right", "Turn corner")], point("slot-right")),
        makeFrame("Pull-up or short roll", "Le 1 choisit le pull-up entre les écrans, la pocket pass ou le short roll selon la couverture du grand défenseur.", [position("o1", "slot-right", 0, 8), position("o4", "block-left"), position("o5", "block-right"), position("o2", "corner-left"), position("o3", "corner-right")], { ...point("slot-right", 0, 8), owner: "o1" }, [action("dribble", "o1", "rim", "Attack gap"), action("roll", "o5", "rim", "Short roll"), action("pass", "o1", "o5", "Pocket pass"), action("shot", "o1", "rim", "Pull-up")], point("block-right")),
      ];
    case "ghost-pnr":
      return [
        makeFrame("Ghost PnR · alignment", "Le 5 part du slot avec une vraie vitesse d’écran. Les corners restent occupés pour empêcher l’aide facile.", [position("o1", "top"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "wing-left"), position("o5", "slot-right")], { ...point("top"), owner: "o1" }),
        makeFrame("Show then ghost", "Le 5 montre l’écran puis s’échappe avant le contact. Le porteur attaque l’espace plutôt que d’attendre une collision.", [position("o1", "slot-right"), position("o5", "wing-right"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "wing-left")], { ...point("slot-right"), owner: "o1" }, [action("screen", "o5", "o1", "Ghost screen"), action("dribble", "o1", "slot-right", "Attack gap"), action("pop", "o5", "wing-right", "Ghost pop")], point("wing-right")),
        makeFrame("Pop or re-attack", "Si le défenseur du porteur passe sous l’action, le 5 reçoit derrière l’arc ; sinon le 1 poursuit vers le cercle.", [position("o1", "rim"), position("o5", "wing-right"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "wing-left")], { ...point("wing-right"), owner: "o5" }, [action("pass", "o1", "o5", "Hit the pop"), action("shot", "o5", "rim", "Pop three"), action("shot", "o1", "rim", "Finish")], point("rim")),
      ];
    case "chicago-action":
      return [
        makeFrame("Chicago · pindown setup", "Le shooteur part bas, le screener est prêt à poser un pindown et le 1 garde la balle assez haut pour voir la sortie.", [position("o1", "top"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "block-left"), position("o5", "wing-right")], { ...point("top"), owner: "o1" }),
        makeFrame("Pindown to DHO", "Le shooteur utilise le pindown puis enchaîne immédiatement avec le handoff. La défense ne doit pas avoir de temps mort entre les deux actions.", [position("o1", "wing-left"), position("o2", "slot-left"), position("o3", "corner-right"), position("o4", "elbow-left"), position("o5", "wing-right")], { ...point("wing-left"), owner: "o1" }, [action("cut", "o2", "slot-left", "Pindown exit"), action("screen", "o4", "o2", "Pindown"), action("pass", "o1", "o2", "DHO entry")], point("slot-left")),
        makeFrame("Read the handoff", "Le shooteur reçoit lancé et lit tir, turn-the-corner ou backdoor. Le premier screener roule pour punir le switch.", [position("o1", "slot-left"), position("o2", "wing-left"), position("o3", "corner-right"), position("o4", "block-left"), position("o5", "wing-right")], { ...point("wing-left"), owner: "o2" }, [action("dribble", "o2", "rim", "Turn corner"), action("roll", "o4", "rim", "Roll"), action("shot", "o2", "rim", "Shot")], point("rim")),
      ];
    case "horns-elbow-split":
      return [
        makeFrame("Horns split · entry", "Les deux bigs occupent les elbows et les corners restent très larges. L’entrée ne révèle pas encore le côté du split.", [position("o1", "top"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "elbow-left"), position("o5", "elbow-right")], { ...point("top"), owner: "o1" }),
        makeFrame("Elbow entry", "Le 1 passe au 4 puis coupe autour de son elbow. Le 5 garde l’autre elbow comme écran ou safety.", [position("o1", "corner-left"), position("o2", "slot-left"), position("o3", "corner-right"), position("o4", "elbow-left"), position("o5", "elbow-right")], { ...point("elbow-left"), owner: "o4" }, [action("pass", "o1", "o4", "Elbow entry"), action("cut", "o1", "corner-left", "Clear out"), action("screen", "o5", "o2", "Split screen")], point("elbow-left")),
        makeFrame("Split read", "Le receveur choisit la sortie extérieure, le handoff ou la coupe backdoor. Le weak-side corner ne remonte pas dans le même espace.", [position("o1", "corner-left"), position("o2", "wing-left"), position("o3", "corner-right"), position("o4", "elbow-left"), position("o5", "block-right")], { ...point("elbow-left"), owner: "o4" }, [action("pass", "o4", "o2", "Split handoff"), action("cut", "o2", "wing-left", "Exit"), action("roll", "o5", "rim", "Slip"), action("shot", "o2", "rim", "Catch and shoot")], point("wing-left")),
      ];
    case "blob-zipper":
      return [
        makeFrame("BLOB zipper · setup", "Le remiseur est sous le panier. Le leurre attaque le cercle tandis que le shooteur part bas pour remonter autour du zipper.", [customPosition("o1", 50, 95, "inbound"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "block-left"), position("o5", "elbow-right")], { x: 50, y: 95, owner: "o1" }),
        makeFrame("Zipper lift", "Le cutter vend la coupe au cercle puis remonte le long de la raquette. Le screener reste stationnaire et ouvre une ligne légale.", [customPosition("o1", 50, 95, "inbound"), position("o2", "slot-left"), position("o3", "rim"), position("o4", "block-left"), position("o5", "elbow-right")], { x: 50, y: 95, owner: "o1" }, [action("cut", "o3", "rim", "Decoy cut"), action("cut", "o2", "slot-left", "Zipper lift"), action("screen", "o4", "o2", "Zipper screen")], point("slot-left")),
        makeFrame("Hit or safety", "Le remiseur sert le shooteur au slot. Si la fenêtre est fermée, la safety au slot droit conserve la possession.", [customPosition("o1", 50, 95, "inbound"), position("o2", "slot-left"), position("o3", "rim"), position("o4", "corner-left"), position("o5", "slot-right")], { ...point("slot-left"), owner: "o2" }, [action("pass", "o1", "o2", "Hit zipper"), action("shot", "o2", "rim", "Quick three"), action("pass", "o1", "o5", "Safety outlet")], point("slot-right")),
      ];
    case "slob-safety":
      return [
        makeFrame("SLOB safety · setup", "Le remiseur est sur la ligne de touche. Deux menaces attaquent le milieu et la baseline, pendant que la safety garde un angle de sortie.", [customPosition("o1", 4, 52, "inbound"), position("o2", "corner-left"), position("o3", "corner-right"), position("o4", "elbow-right"), position("o5", "slot-right")], { x: 4, y: 52, owner: "o1" }),
        makeFrame("Two threats, one safety", "Le premier cutter attire l’aide et le second coupe dans le dos. La safety remonte tôt, avant que la trap ne ferme la ligne.", [customPosition("o1", 4, 52, "inbound"), position("o2", "wing-left"), position("o3", "rim"), position("o4", "elbow-right"), position("o5", "slot-left")], { x: 4, y: 52, owner: "o1" }, [action("cut", "o2", "wing-left", "Middle release"), action("cut", "o3", "rim", "Backdoor"), action("move", "o5", "slot-left", "Safety lift")], point("slot-left")),
        makeFrame("Safety pass", "Le remiseur passe à la première fenêtre sûre. Si le corner est fermé, l’outlet au slot permet de réinitialiser sans perte de balle.", [customPosition("o1", 4, 52, "inbound"), position("o2", "wing-left"), position("o3", "rim"), position("o4", "corner-right"), position("o5", "slot-left")], { ...point("wing-left"), owner: "o2" }, [action("pass", "o1", "o2", "Hit receiver"), action("shot", "o2", "rim", "Catch and shoot"), action("pass", "o1", "o5", "Safety outlet")], point("slot-left")),
      ];
    case "blob-box-lob":
      return [
